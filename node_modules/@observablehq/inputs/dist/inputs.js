...
function createCheckbox(multiple, type) {
  return createChooser({
    ...
    render(data, index, selected, disabled, {format, label}) {
      const form = htl.html`<form class="inputs-3a86ea inputs-3a86ea-checkbox">
...
function toggle({label, value, values, disabled} = {}) {
  const input = htl.html`<input class=inputs-3a86ea-input type=checkbox name=input disabled=${disabled}>`;
  const form = htl.html`<form class="inputs-3a86ea inputs-3a86ea-toggle">${maybeLabel(label, input)}${input}`;
...
function text({
  ...
  >`;
  const form = htl.html`<form class=inputs-3a86ea style=${maybeWidth(width)}>
...
const select = createChooser({
  render(data, index, selected, disabled, {format, multiple, size, label, width}) {
    const select = htl.html`<select class=inputs-3a86ea-input disabled=${disabled === true} multiple=${multiple} size=${size} name=input>
    ...
    const form = htl.html`<form class=inputs-3a86ea style=${maybeWidth(width)}>${maybeLabel(label, select)}${select}`;
    return [form, select];
  },
  selectedIndexes(input) {
...
function table(data, options = {}) {
  ...
  const id = newId();
  const root = htl.html`<form class="inputs-3a86ea inputs-3a86ea-table" id=${id} style=${{height: length(height), maxHeight: length(maxHeight), width: typeof width === "string" || typeof width === "number" ? length(width) : undefined, maxWidth: length(maxWidth)}}>`;
...
function initialize(
  ...
  const tr = htl.html`<tr><td>${selectable ? htl.html`<input type=${multiple ? "checkbox" : "radio"} name=${multiple ? null : "radio"}>` : null}</td>${columns.map(() => htl.html`<td>`)}`;
  const theadr = htl.html`<tr><th>${selectable ? htl.html`<input type=checkbox onclick=${reselectAll} disabled=${!multiple}>` : null}</th>${columns.map((column) => htl.html`<th title=${column} onclick=${event => resort(event, column)}><span></span>${header && column in header ? header[column] : column}</th>`)}</tr>`;
...
function textarea({
  ...
  >`;
  const form = htl.html`<form class="inputs-3a86ea inputs-3a86ea-textarea" style=${maybeWidth(width)}>
